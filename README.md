...
│       └── extraction.py # Pydantic structured output extraction controller
│
└── static/              # Frontend static web directory
    ...
    └── js/              # ES6 Modular Javascript files
        ...
        ├── extraction.js # Structured Pydantic extraction managers
...
Explore six specialized Large Language Model (LLM) and LangChain modules inside the dashboard:
...
### 2. 📄 Retrieval-Augmented Generation (RAG) & Chunk Visualizer
...
* Run semantic context queries and watch relevant source chunks instantly highlight on a real-time **Interactive RAG Block Visualizer** grid map.
* Hover over chunk card nodes to inspect exact index positions and absolute similarity score percentages (e.g., `92.4% Similarity`).
...
### 6. 📋 Structured Data Extractor (Pydantic Showcase)
* Extract clean, structured JSON schemas from completely raw, unstructured text.
* Supports three production-grade schema configurations:
  1. **Receipts & Invoices**: Merchant name, itemized pricing lines, tax, date, and final totals.
  2. **Contact Cards**: Names, phone records, dynamic email lists, and corporate titles.
  3. **Event Calendars**: Dynamic meeting subjects, start/end timestamps, physical/virtual venues, and guest list rosters.
* Visualizes extracted outputs in an interactive, responsive grid table with one-click **CSV** and **JSON** local dataset exports.
